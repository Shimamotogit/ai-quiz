...
>
> `音声データ_MP3/` 以下の MP3 は、リポジトリ管理者の申告に基づき、すべて VOICEVOX:春日部つむぎ で生成された音声です。詳細は [`docs/AUDIO_LICENSE.md`](docs/AUDIO_LICENSE.md) を確認してください。
...
提供されたAIクイズ問題マスタ50問のうち、組み込み出題では6問を除外しています。

- No.12: 実際の画像・写真の提示を前提とする問題のため除外
- No.21〜24: 回答未設定
- No.26: 〇✕形式に対して回答が `A`

既定の出題可能問題数は44問です。組み込み問題では、`image` が指定された問題や「この画像」「この写真を見て」など実際の画像・写真を見ないと成立しない問題を出題対象から外します。一方、「写真に写っている犬と猫を見分けられるAIがある」のように、写真について説明するだけで画像提示を必要としない文章問題は出題対象に残します。

また、No.27「生成AIに与える指示文や質問テキストのことを『プロンプト』と呼ぶ。」は〇✕問題として出題し、正解は〇です。
...
`image`、`audio`、`explanationAudio` は同一サイト内の相対パスのみ利用できます。ローカルJSONで音声を明示していない場合、組み込み問題用の連番MP3へ自動対応せず、ブラウザ読み上げへフォールバックします。ローカルJSONは利用者が指定した内容を尊重するため、組み込み問題向けの画像問題除外やNo.27補正は適用しません。
...
このアプリで配布・再生する `音声データ_MP3/` 以下のMP3は、リポジトリ管理者の申告に基づき **VOICEVOX:春日部つむぎ** で生成されています。
...
春日部つむぎ公式利用規約では `VOICEVOX:春日部つむぎ` のクレジット表記が案内され、商用・非商用利用が可能とされています。一方で、禁止事項には `二次配布` も含まれます。生成済みMP3を公開リポジトリに含める配布形態について、本READMEでは独自に許諾済みとは判断しません。公開・再配布時は最新の公式規約を確認してください。
...
GitHub Actionsでは、JavaScript構文、画面遷移、回答表示、音声フロー、既定値、説明画像、組み込み問題の〇✕化・画像問題除外、VOICEVOXクレジット・規約案内などを検証します。音声ファイルそのものは差し替え可能なため、固定ハッシュによる内容一致検証は行いません。
...
- VOICEVOX・春日部つむぎ関連の規約は更新される可能性があるため、公開・再配布・商用利用前に最新版を確認してください。
