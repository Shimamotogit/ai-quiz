...
import {
  getKidsHiraganaQuestion,
  getKidsHiraganaQuestionCount,
  isCircleCrossQuestion
} from "../quiz-presentation.js";
...
const presentationSource = await readFile(new URL("../quiz-presentation.js", import.meta.url), "utf8");
const presentationCss = await readFile(new URL("../quiz-presentation.css", import.meta.url), "utf8");
const mainSource = await readFile(new URL("../main.js", import.meta.url), "utf8");
const indexSource = await readFile(new URL("../index.html", import.meta.url), "utf8");

for (const text of [
  "ルールの説明です。",
  "問題の出題後、〇・×、正解だと思う位置に移動してください。",
  "一度移動した後は、そのままの状態をキープしてください。"
]) {
...
assert.ok(presentationInit >= 0 && systemAudioInit >= 0 && presentationInit < systemAudioInit,
  "全画面開始ハンドラがシステム音声ガードより先に初期化されていません");
